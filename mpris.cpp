...
MprisControlPlugin::MprisControlPlugin(QObject *parent)
    : sessionManager(NULL) {
    try {
        sessionManager = GlobalSystemMediaTransportControlsSessionManager::RequestAsync().get();
        if (sessionManager) {
            sessionManager.SessionsChanged(
                [this](GlobalSystemMediaTransportControlsSessionManager, SessionsChangedEventArgs) {
...
// }
//
void MprisControlPlugin::sendMediaProperties(const QString &name,
    ...
    try {
        ...
        qDebug() << "Failed to get media properties";
    }
    ...
    sendTimelineProperties(name, player, true); // "length"
}

void MprisControlPlugin::sendPlaybackInfo(const QString &name,
                                          const GlobalSystemMediaTransportControlsSession &player) {
    ...
    sendMediaProperties(name, player);
    ...
    qDebug() << "isPlaying:" << int(
        playbackInfo.PlaybackStatus() == GlobalSystemMediaTransportControlsSessionPlaybackStatus::Playing);
    qDebug() << "canPause:" << playbackControls.IsPauseEnabled();
    qDebug() << "canPlay:" << playbackControls.IsPlayEnabled();
    qDebug() << "canGoNext:" << playbackControls.IsNextEnabled();
    qDebug() << "canGoPrevious:" << playbackControls.IsPreviousEnabled();
    qDebug() << "canSeek:" << playbackControls.IsPlaybackPositionEnabled();
    ...
    if (playbackInfo.AutoRepeatMode()) {
        ...
        qDebug() << "loopStatus:" << loopStatus;
    }

    sendTimelineProperties(name, player);

    // if (packetOrName.index() == 1)
    //     sendPacket(np);
}

void MprisControlPlugin::sendTimelineProperties(const QString &packetOrName,
                                                const GlobalSystemMediaTransportControlsSession &player,
                                                bool lengthOnly) {
    // NetworkPacket np = packetOrName.index() == 0 ? std::get<0>(packetOrName) : NetworkPacket(PACKET_TYPE_MPRIS);
    qDebug() << "player:" << packetOrName;
    ...
    if (!lengthOnly) {
        ...
        qDebug() << "canSeek:" << playbackControls.IsPlaybackPositionEnabled();
        qDebug() << "pos:" << std::chrono::duration_cast<std::chrono::milliseconds>(
    ...
    try {
        ...
        for (uint32_t i = 0; i < sessions.Size(); i++) {
            ...
            try {
                ...
                qDebug()
                        << QString::fromWCharArray(playerName.c_str()) <<
                        "doesn\'t have a valid AppUserModelID! Sending as-is..";
            }
            QString origName = QString::fromWCharArray(playerName.c_str());
            QString uniqueName = origName;
            for (int i = 2; playerList.contains(uniqueName); ++i) {
                uniqueName = origName + QStringLiteral(" [") + QString::number(i) + QStringLiteral("]");
            }
            ...
            player
                    .PlaybackInfoChanged(auto_revoke,
                                         [this](GlobalSystemMediaTransportControlsSession player,
...
//         qWarning(KDECONNECT_PLUGIN_MPRISCONTROL) << "Unable to get thumbnails: " << QString::fromWCharArray(
//             ex.message().c_str());
//         return;
//     }
//
...
// }
//
// bool MprisControlPlugin::sendAlbumArt(const std::variant<NetworkPacket, QString> &packetOrName,
//                                       const QSharedPointer<QBuffer> qdata, const QString artUrl) {
...
//
//         if (action == QStringLiteral("PlayPause") || (action == QStringLiteral("Play")) || (
//                 action == QStringLiteral("Pause"))) {
